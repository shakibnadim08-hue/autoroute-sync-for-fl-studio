...
def _selected_channel_is_automation_clip():
    """Return True while the selected Channel Rack item is an Automation Clip."""
    try:
        selected = int(channels.selectedChannel(1, 0, 1))
    except Exception:
        return False
    return selected >= 0 and _is_automation_clip_channel(selected)


def _sync_active_pattern_color():
    """Give a new/default active pattern a non-red channel-derived color.

    Pattern edits can generate dirty-pattern refreshes while an Automation Clip
    is being drawn. Do not touch pattern state during that edit session.
    """
    global _last_active_pattern, _managed_pattern_colors

    if _selected_channel_is_automation_clip():
        return
    ...
    # Dirty-pattern callbacks can arrive repeatedly. Avoid writing the same
    # color back to FL Studio, since that unnecessary UI mutation can repaint
    # the Playlist while the user is editing an Automation Clip.
    if current_color == int(color):
...
def _is_automation_clip_channel(channel_index):
    """Return True for FL Studio Automation Clip channels."""
    try:
        return int(channels.getChannelType(channel_index, True)) == getattr(midi, "CT_AutoClip", 5)
    except Exception:
        return False


def _sync_new_channel(channel_index, occupied):
    """Color and route one newly created audio/instrument channel."""
    if _is_automation_clip_channel(channel_index):
        _log("Skipped Automation Clip channel %d; no routing or color assignment" % channel_index)
        return
